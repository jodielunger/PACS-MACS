...
2. `run_two-state_model_v3.Rmd`: This is the only script required to run the two-state model. This script is best run in Rstudio or another UI optimized for running R markdown files. Note if you do not already have all required R packages, you may need to install them before they can be loaded (first code chunk).
---
...
4. **Copy-number scaling.** A motif present in $n$ copies contributes $n \times \Delta E\text{s}$ to the total energy, i.e., contributions scale linearly with copy number.
...
> ```math
> f_{pred} = f_{low} + \frac{f_{high} - f_{low}}{1 + \exp\left(\Delta E\text{i} + \sum_{j=1}^{9} \Delta E\text{s}_{j} \cdot n_j\right)}
> ```
...
For each phenotype, the 10 model parameters ($\Delta E\text{i}$ and $\Delta E\text{s}_{j}$ for $j = 1 \ldots 9$) were estimated by minimizing the sum of squared residuals in fluorescence space over all individual replicate measurements:
...
- **CV-SD**: the standard deviation of each $\Delta E\text{s}_{j}$ estimate across the 10 folds, used as the error bar on $\log_{10}(K_s)$ plots.

---

## Converting $\Delta E$ to Equilibrium Constants

Each fitted $\Delta E\text{s}_{j}$ is converted to a per-motif equilibrium constant contribution:

> ```math
> K_{s,j} = e^{-\Delta E\text{s}_{j}}
> ```
...
> ```math
> \text{SD}[\log_{10}(K_s)] = \log_{10}(e) \times \text{SD}[\Delta E\text{s}]
> ```
